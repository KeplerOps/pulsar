...
| Risk | Mitigation |
|------|-----------|
| Future presenter-interrupt support might need state the resolver currently does not own. | The orchestrator's small surface (one async function, two callbacks) is easy to extend; cancellation can land as an `AbortSignal` option without changing the failure semantics. Re-evaluate when PUL-F005+ specifies presenter controls. **Update (PUL-F006):** the `AbortSignal` seam landed early. `ResolveCompositionOptions.signal` and `SceneTimelineRunInput.signal` now exist; the resolver checks the signal pre-start and between scenes, and forwards it to the runner. Failure semantics are unchanged: a runner that throws in response to an abort still routes through the cleanup-always invariant, and inter-scene aborts throw with the previously-completed scene id named in the message. PUL-F020 (wave-1 presenter controls) wires the actual presenter UI to a controller that owns this signal. |
