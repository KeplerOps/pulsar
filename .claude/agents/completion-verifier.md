...
Run `git diff --name-only HEAD` to see what files have been modified. Determine whether any source files were changed (not just GC MCP operations or documentation-only changes). Source files include `.js`, `.ts`, `.css`, `.html` (anything implementing runtime behavior). Documentation-only changes (`docs/**/*.md`) and GC MCP-only operations do not count.
...
Look at the git log and branch name to determine if a requirement is being implemented (branch names typically contain issue numbers, and the conversation would reference PUL-* UIDs).
...
```json
{"ok": false, "reason": "Missing: CHANGELOG.md not updated despite source code changes in src/..."}
```
